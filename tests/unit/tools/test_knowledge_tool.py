...
def test_k_bounds_the_candidates_not_the_passage_count(corpus_retriever):
    ...
    passages = search_plant_knowledge(corpus_retriever, queries, k=4)

    assert len({p.doc_id for p in passages}) <= 4
    assert len(passages) <= 8


def test_the_shortlist_is_ranked_and_comes_first(corpus_retriever):
...
def test_one_passage_per_disorder_in_the_shortlist(corpus_retriever):
...
def test_every_shortlisted_disorder_contributes_its_look_alikes(corpus_retriever):
...
def test_no_passage_is_returned_twice(corpus_retriever):
...
def test_search_with_no_queries_returns_empty(corpus_retriever):
    assert search_plant_knowledge(corpus_retriever, [], k=4) == []


def test_only_descriptive_sections_are_allowed_to_match(corpus_retriever):
    ...
    class _Recorder:
        def search(self, queries, k, *, sections=None):
            ...
            return corpus_retriever.search(queries, k, sections=sections)

        def sections_for(self, doc_ids, sections):
            return corpus_retriever.sections_for(doc_ids, sections)
...
def test_hypothesised_documents_arrive_regardless_of_rank(corpus_retriever):
    ...
    passages = search_plant_knowledge(
        corpus_retriever, queries, k=4, hypotheses=["potassium-deficiency"]
    )

    assert "potassium-deficiency" in {p.doc_id for p in passages}


def test_a_hypothesised_document_brings_its_descriptive_sections(corpus_retriever):
    ...
    passages = search_plant_knowledge(
        corpus_retriever, [], k=4, hypotheses=["potassium-deficiency"]
    )
...
def test_a_hypothesis_that_similarity_also_found_is_not_duplicated(corpus_retriever):
    queries = build_symptom_queries(_symptoms(), species=None)
    matched = search_plant_knowledge(corpus_retriever, queries, k=4)
    found_anyway = next(p.doc_id for p in matched)

    passages = search_plant_knowledge(corpus_retriever, queries, k=4, hypotheses=[found_anyway])

    keys = [(p.doc_id, p.section) for p in passages]
    assert len(keys) == len(set(keys))


def test_hypotheses_alone_are_enough_to_retrieve(corpus_retriever):
    ...
    passages = search_plant_knowledge(corpus_retriever, [], k=4, hypotheses=["root-rot"])

    assert {p.doc_id for p in passages} == {"root-rot"}


def test_neither_queries_nor_hypotheses_returns_nothing(corpus_retriever):
    assert search_plant_knowledge(corpus_retriever, [], k=4, hypotheses=[]) == []
