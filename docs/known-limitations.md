...
| U3 | **~~Uploads are not re-encoded or downscaled.~~** *Resolved 2026-09-04.* `core/images.py::downscaled` caps a photograph's long edge at a new `max_image_edge_px` setting (1568px by default, Lanczos-resampled), called from `store_upload` between `upright_bytes` and `without_position` so a photograph already inside the cap comes back byte-identical. The row's own diagnosis was already half stale before the fix started: "Pillow is a dependency but unused" stopped being true the day `upright_bytes` began calling `ImageOps.exif_transpose` for orientation — Pillow was doing rotation, only not size. | The real cost wasn't the four lines in `core/images.py` — it was that capping what the vision model sees falsifies a live requirement. `openspec/specs/photo-storage/spec.md` said in as many words that the system SHALL NOT downscale, recompress or crop; shipping this without correcting that spec would have left it arguing against the code it describes, so the amendment below (Requirement: The model sees what the owner photographed) is part of this fix, not a separate task. | ✓ Complete. `max_upload_bytes` (8 MB) and `max_images_per_observation` (4) are separate limits — file size and count, not pixels — and this change leaves both untouched. |
...
| U8 | **~~The upload-rejection message interpolates the model's description raw~~**, *Resolved 2026-09-04.* `agent/nodes/intake.py::rejection_message` strips the model's own full stop and lower-cases the first letter — but only when the description reads as ordinary prose. A two-character test (`described[:1].isupper() and not described[1:2].isupper()`) leaves an acronym alone, so "NASA logo" stays "NASA logo" instead of becoming "nASA logo". The fix this row asked for — "lowercase the first character" — was the wrong diagnosis taken literally; it would have broken on the first acronym the gate model returned. | The same function now also takes `app_title` from settings instead of a literal, so this fix doubles as half of `M13` below rather than sitting beside it as an unrelated change. | ✓ Complete. Two tests: one for the ordinary-sentence case, one asserting the acronym stays capitalised — a plain `.lower()` would have passed the first and failed the second. |
...
---
| U20 | **Nothing this application sends ever leaves the machine.** No mail provider is configured and none will be before review, because sending to an arbitrary address needs a verified sender domain and there is none. `core/mail.py` falls back to writing the whole message to the application log, link included — so registration and password reset both work, and both finish in a terminal rather than an inbox. | A provider key alone does not fix it: Resend's default sender only delivers to the address that owns the Resend account, so a reviewer would still receive nothing while the developer received everything — the same symptom with a more confusing cause. Writing to the log is honest by comparison, and the screens now say so rather than claiming a message was sent. | Verify a sender domain with a provider, set `PLANTOPIA_RESEND_API_KEY` and `PLANTOPIA_MAIL_FROM`, and the fallback stops being used with no other change. Until then the README's "Getting an account" section is the path: register, then take the link from the terminal running uvicorn. |
| U21 | **~~Removing a plant redirected to its own 404 instead of the plants list.~~** *Found and fixed 2026-09-04, arrives here already resolved.* `useRemovePlant`'s success handler invalidated `keys.plants` without `exact: true`, and `keys.plants` is a prefix of `keys.plant(id)` — TanStack Query's default prefix match refetches every query keyed under `keys.plant(id)` too, for a plant that no longer exists. The cause generalises: adding a query key under an existing prefix inherits this trap unless `exact: true` is reached for deliberately. | Fixed in `web/src/api/hooks/plants.ts`: `useRemovePlant` now takes `plantId` directly (closing over it, matching `useRenamePlant`/`useMarkStep` beside it) and invalidates with `exact: true`; `PlantDetail.tsx` now navigates to `/` with `replace: true` on success instead of round-tripping `plantId` through `mutate`. | ✓ Complete. The regression test doesn't assert on the DOM directly — TanStack Query swallows a failed background refetch internally, so a DOM-only assertion passes against the unfixed code too. It asserts the detail endpoint is never requested again after removal, the one thing that actually discriminates old from fixed. |
| U25 | **~~Every interrupted diagnosis was charged for only the pass that finished it.~~** *Found and fixed 2026-09-07, arrives here already resolved.* This row said "roughly half price" until the first real diagnosis after the wipe measured it: the pass before the pause was $0.0045781 of a $0.0270073 run, so the ledger was short by 17% of cost and 35% of prompt tokens rather than by half. One run, but the only measurement that exists — the figure is unobtainable from tests, for the reason the third column gives. `U26` is the same undercount one layer up, missed by this fix. A run is driven in two passes — the first stops at the mandatory clarifying-question interrupt, the second resumes and finishes it — and each pass builds its own `UsageCollector` in its own worker thread. `runs/worker.py::_record_usage` returned early on a non-terminal status and then recorded only the finishing pass's snapshot, on the belief stated in its own docstring that "the collector is scoped to the thread, so the pass that finishes reports the whole thing". It never was. LangGraph replays completed nodes from the checkpoint rather than re-running them, so the vision and gate calls — which `core/cost.py` notes are the expensive ones, and which all happen before the pause — were counted nowhere. That figure feeds `UsageRepository.record`, which `services/limits.py:122` sums for the per-owner quota and the daily spend cap, so both were enforced against an understated total. A run abandoned at the interrupt was free outright: the sweeper recorded a null cost for it. | Fixed rather than carried, because it is money and it was silent. The pause writes its snapshot to a new `runs.partial_usage_json`, and the pass that finishes adds it with `UsageSnapshot.plus` — so one ledger entry per run still holds and the entry is now the whole run. Durable rather than in memory because the pause lasts as long as somebody takes to answer, which can outlive the process. The sweeper records what was carried, and null only when nothing was. | **Why nothing caught it, which is the part worth keeping.** The end-to-end tests asserted *whether* usage was recorded and never how much — and the scripted models report no usage at all, so `snapshot()` returned `None` throughout and every figure looked alike. Three tests now report usage on purpose and were confirmed to fail against the old code (`0 == 1000`) before the fix landed. Found while removing the dead `DiagnosisService`, which got this right: it kept a collector per thread id. The background-run rewrite inherited its docstring and lost its invariant. |
| U26 | **~~The diagnosis record kept the undercount `U25` fixed in the ledger.~~** *Found and fixed 2026-09-07, hours after `U25`.* `cba92c0` corrected `usage_events`, which the worker writes once the graph returns, and left `diagnoses.token_usage_json` and `diagnoses.cost_usd` alone — `agent/nodes/persist` reads the run-scoped `UsageCollector`, and a resumed run gets a new collector in a new worker thread, so the row held the finishing pass while the ledger held the run. Measured on the same diagnosis that corrected `U25`'s headline: ledger 10367/1478 tokens at $0.0270073, row 6728/1069 at $0.0224292 — exactly the ledger less the $0.0045781 the pause had banked. The two disagreed about one run by 17% of its cost. | Fixed, on the same grounds as `U25`: it is money and it was silent. The pass that finishes is now told what earlier passes spent through `config.configurable.carried_usage`, and `_usage_from` adds it the way `_total_spend` already does for the ledger. Passed through the config rather than `UPDATE`d afterwards because `persist.py`'s docstring argues that case already: the figure is written inside the graph's transaction so a crash between two writes cannot leave a diagnosis with a NULL cost, which is `M12`. | **The lesson is about where a fix stops.** `U25` was diagnosed correctly and fixed at one of its two call sites, because the investigation followed the money to the quota and the spend cap and stopped there — the record was never asked. `_usage_from`'s docstring carried the same false invariant as `_record_usage`'s, word for word in substance ("the snapshot is final"), and nothing flagged the duplicate. Found by reading `diagnoses` in the database while confirming `U25`'s own fix, which is the only place either was visible. Also worth noting for `U23`: its stated plan is to render `diagnosis.cost_usd` as a cost badge, which until now would have shown the understated figure. |
| U22 | **`Differential.tsx:29` re-sorts the candidates the server already ranked.** `[...diagnosis.candidates].sort((a, b) => b.probability - a.probability)` imposes the client's own order on a list `diagnose` already ordered. The two rules likely agree today, so nothing has been seen to move — but if they ever disagree, the screen shows a different leading candidate than the diagnosis actually recorded, and `eval/` scores the server's own order, so no measurement would see the screen disagree with it. Found while rewriting `docs/code-tour.md` against the real source (Task 6), not while looking for it. | Not fixed here — it is a one-line change with no failing test to drive it, and writing one first means deciding what should happen when the two orders *do* disagree, which nobody has decided. | Drop the `.sort(...)` and render `diagnosis.candidates` in the order the API returns it, or add a test that pins the two orders together so a future divergence fails loudly instead of silently. |
| U23 | **Three things the Streamlit diagnosis page showed have no React equivalent, and nobody decided to drop them — they just weren't rebuilt.** The retired `ui/pages/diagnose.py` (deleted retiring Streamlit) had a "Sources consulted" expander naming every retrieved passage and visual match, a "What the agent did" expander listing `tools_used` and any non-fatal errors, and `render_cost_badge` showing tokens and cost at the foot of every result. Checked against the current tree: `retrieved`, `visual_matches` and `tools_used` never reached `DiagnosisOut` at all — no schema or router mentions them, so that gap is at the API boundary, not just the screen. `cost_usd` fares differently: it is exposed on `DiagnosisOut` (`api/schemas.py:47`) and typed on the client (`web/src/api/types.ts:186`), and a grep for `cost_usd`/`token_usage` outside `api/types.ts` in `web/src` finds nothing that renders it — a different problem from `M18`, where a failed run records no cost at all; here the cost *is* recorded and simply never shown. Found while rewriting `docs/code-tour.md` against the real source (Task 6). | Not fixed here. Each is a real design decision nobody has made — how much retrieval detail an owner wants to see, whether tool-call detail belongs on the result screen, where a cost badge earns its place next to the differential — and the migration simply didn't carry the old page's contents across item by item rather than deciding against them. | For the cost badge: render `diagnosis.cost_usd` on `Differential`, on `render_cost_badge`'s own rule of showing nothing rather than "$0.0000" when it is `null`. For the other two: decide first whether an owner should see retrieval provenance and tool-call detail at all — Streamlit showed both by default — then add `retrieved`/`visual_matches`/`tools_used` to `DiagnosisOut` before there is anything for a screen to render. |
| U24 | **`Differential.tsx` dropped the "nothing argues against it" caption the Streamlit view had.** The retired `ui/components/differential.py` rendered `st.caption("Nothing observed argues against this.")` whenever a candidate's `contradicting_evidence` came back empty — deliberately asymmetric with "Points to it", which had no such fallback, because a candidate with no supporting evidence would be strange and one with no counter-evidence is ordinary. `Differential.tsx:78-87` instead renders the whole "What argues against it" heading and list only when `candidate.contradicting_evidence.length` is truthy, exactly the same rule already applied to "What points to it" — so the one case Streamlit called out explicitly is now silent like every other empty list, and a reader sees two headings with a gap where a third used to say plainly that the gap was checked and found empty, not merely omitted. Confirmed by reading both files; found while rewriting `docs/code-tour.md` against the real source (Task 6). | A copy regression rather than a data loss — `contradicting_evidence` is exactly as empty either way — and nothing caught it because no test pinned the old wording. | Give the empty case its own branch again: render "Nothing observed argues against this" (or equivalent) when `candidate.contradicting_evidence.length === 0`, rather than letting it fall into the same `null` that silences "points to it". |
...
| M13 | **The row originally said one hardcoded `"Plantopia"`; a first accounting found seven in Python and four more in a parallel TypeScript table, missing `core/config.py`, `identity/messages.py` and `api/main.py` entirely. Amended 2026-09-04 with an exact recount: pre-fix there were 16 hardcoded `"Plantopia"` occurrences in Python; 13 remain, in four groups.** Fixed by this branch: `agent/nodes/intake.py` (1, folded into `U8` since both needed the same function touched) and `api/errors.py` (2, the busy-queue and daily-cap error details, fixed by Task 7's `register(app, settings)`) — both now zero. | Remaining, by group and disposition: `services/chat_events.py` (**4**, `TOOL_NAMES`) is **blocked** — `web/src/screens/chat/sources.ts:10-16` holds the identical four strings as a static TypeScript literal, and `tests/api/test_source_names_agree.py` regex-parses that file to assert the two tables agree; a dynamic server side would agree with a client that has no settings concept at all only by coincidence, so closing this means serving the title to the client first. `identity/messages.py` (**5**: email subjects and bodies, lines 34, 35, 51, 52, 64) is **not blocked** — that module already takes `base_url` as a parameter, the same shape of change `api/errors.py` just had; it is left because it is user-facing email copy that wants its own change and its own tests, not because it is hard. `api/main.py:36` (**1**, `title="Plantopia"`) is **deliberate** — that is the OpenAPI document's title, not user-facing copy. `core/config.py` (**3**: `app_title`'s default, the user-agent string, `mail_from`) is **deliberate** — these are the definitions everything else reads from. | Serve the title to the client — an endpoint, or bake it in at build time — so `chat_events.py` and `sources.ts` can agree on one variable instead of one literal; only then does making `chat_events.py` dynamic mean anything. `identity/messages.py` is a separate, smaller task: thread `app_title` through its subjects and bodies the way `api/errors.py` just did, with tests asserting on the rendered text. |
...
| M53 | **Bounding the chat context saves money nobody can see.** `M17` still stands: chat turns write no usage rows, so the reduction — measured at 83% of the tokens billed across a forty-turn conversation — is provable in a test and invisible in production. | Tracking chat usage is a change of its own, and this one is verified from the other side: a test asserts what actually reaches the model, which is the same property measured before it becomes a bill. | Whatever closes `M17` closes this too. The run pipeline already has a `UsageCollector`; chat does not use it. |
| M54 | **The placeholder left in place of cleared tool output does not say what was cleared.** It reads `[cleared]`, so a model can tell something was fetched and not what. Naming the tool would tell it more — "you already looked up the weather" is more useful than "you already looked something up". | It also costs tokens on every cleared entry, for something the model can usually infer from the tool call itself, which is deliberately kept. Left at the default until a real long conversation shows the model asking for something it had already fetched. | Not achievable as written. `[cleared]` is the default of LangChain's `ClearToolUsesEdit`, whose signature is `placeholder: str = '[cleared]'` — a static string with no per-call hook, so the placeholder cannot name the tool it replaced. The only available change is a longer static string, which tells a model nothing more. Verified 2026-09-04. |
| M55 | **The summariser does not know what the conversation is about.** The agent's own prompt carries the plant, its species and its latest diagnosis; the summarisation middleware gets the default extraction prompt and none of that context. | It is an extraction rather than a judgement, and the default prompt is written for exactly that. It also fires only after hundreds of turns, so nothing has yet been condensed by it in practice — tuning a summariser against conversations nobody has had would be guessing. | `SummarizationMiddleware(summary_prompt=...)` takes a custom prompt. Worth writing one the first time a condensed conversation reads badly, which is also the first time there will be one to read. |
...
| M52 | **~~The Node version is pinned in CI and nowhere else.~~** *Resolved 2026-09-04.* Pinned in three places that now have to agree: `.github/workflows/ci.yml` (`node-version: 22`), `web/.nvmrc` (`22`), and `web/package.json`'s new `"engines": {"node": ">=22"}`. This answers the open question `openspec/changes/archive/2026-09-01-add-continuous-integration/design.md:140` deliberately left open — "whether to pin the Node version in the repository rather than only in CI" — without editing that file, which is Tier 3 and stays as a dated record of what was true on 2026-09-01. | The comment above `actions/setup-node` in `ci.yml` said, correctly at the time, that nothing in the repository pins Node — which this fix's own diff made false the moment it landed beside that comment. Caught in review and rewritten to describe the arrangement that now exists (three places, `ci.yml` the one that gets believed when they disagree) rather than the one this change replaced. | ✓ Complete. |
...
| M50 | **~~`recheck_thread` has no production caller and has not had one since the Streamlit UI was deleted.~~** *Resolved 2026-09-04.* Deleted from `agent/threads.py`; the `U7` collision history it carried moved into the module docstring so the reasoning survives the function that caused it. | Five of the file's nine tests had `recheck_thread` as their sole subject and were deleted with it; a sixth lost one assertion line. One was deliberately kept as-is: `test_an_unrecognisable_handle_belongs_to_nobody` still parametrises a hardcoded `"recheck-3-4-0"` string shaped like the old scheme, because `owner_of` refusing to resolve an old-style id by accident is exactly the property worth testing, with or without anything left that produces one. Net five tests gone, coverage essentially unmoved (95.36% → 95.35%). | ✓ Complete. The row's own prediction held exactly: `owner_of`/`verify_owner` coverage survived by trimming call sites rather than deleting tests wholesale. |
...
| M56 | **LangSmith tracing never runs in production.** `core/tracing.py::configure_tracing` has exactly one caller — `eval/run_eval.py:227`. `api/main.py` never calls it: nothing in `create_app` or the request path mentions tracing at all. Setting `PLANTOPIA_LANGSMITH_API_KEY` configures tracing for the evaluation harness and for nothing else — a diagnosis actually served to an owner is never traced — though the setting reads as though it switches tracing on for the application. Found while rewriting `docs/code-tour.md` against the real source (Task 6), not planned for. | Recorded rather than fixed here — wiring it means deciding whether every request should be traced or only a sampled fraction, a cost and privacy question this change has no mandate to answer on its own. It is the more serious of the two gaps this task turned up unplanned: an observability feature that reads as wired and is not is worse than one openly absent, because a maintainer trusting the setting name has no reason to go looking further. | Call `configure_tracing(settings)` once during `create_app`, behind whatever sampling or cost guard the question above settles on. |
