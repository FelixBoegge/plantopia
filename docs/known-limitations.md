...
| M28 | **Rate limiting is per process, not per deployment.** The counters in `api/rate_limit.py` live in this process's memory, so two workers behind one address permit twice the configured limit, and a restart clears every counter. The source is also whatever `request.client.host` reports, which behind a proxy is the proxy — making the limit global rather than per-source until forwarded headers are configured. | Deliberate for a single container. A shared store is a dependency, an availability question and a failure mode of its own, and none of that is worth carrying before there is a second process. Both failures are in the direction of refusing too much or protecting too little against a distributed attacker, not of letting one source through unchecked. | Move the window into Redis, or into the reverse proxy, when there is more than one process. Configure trusted forwarded headers in the deployment change — that one matters sooner, because a proxy makes every visitor share one budget. |
| M29 | **The quota guard and usage recording are complete, tested, and unreached.** `services/limits.check` and `UsageRepository.record` exist and are covered, and nothing calls either: the endpoints that start a run do not exist yet. | Deliberate. The run endpoints belong with the change that makes runs background and streams them, and wiring the guard into a synchronous endpoint built to be deleted would have meant writing it twice. | Wire both into the run endpoints in `add-background-runs-streaming`, which carries it as a task. `tests/unit/services/test_limits_are_not_wired_yet.py` fails the moment anything calls the guard, which is the reminder to delete that test and the note in `services/limits.py` together. |
