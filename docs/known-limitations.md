...
| # | Limitation | Why it was carried | Fix if it matters |
|---|---|---|---|
| M1 | **~~Coverage margin is ~5 statements wide.~~** *Resolved 2026-08-11.* The `ui/bootstrap.py` omission (2026-08-10) bought headroom but the margin kept shrinking as `ui/pages/*` grew at a structural 0% under the default run; adding `ui/pages/my_plants.py` tripped it again (84.76%). Fixed as M1 itself prescribed: `-m ui` is no longer excluded from `addopts`, so `uv run pytest` now exercises the page and component tests too. Total coverage jumped to ~97% since those files are actually well-tested, just previously deselected. | Folding the marker in was the documented next step, and it is more honest than omitting more files — the pages are genuinely covered, so the number should say so. | `uv run pytest -m ui --no-cov` still isolates just the Streamlit tests when that is useful on its own. |
