...
| M27 | **Endpoint tenancy is exercised below HTTP.** Every route that names a resource is asked once as its owner and once as somebody else (`tests/api/test_endpoint_tenancy.py`, 8 routes), and the second owner arrives by overriding the owner dependency rather than by issuing a request as a different person — because with one seeded owner there is no second person to issue it as. | The half that can be wrong is exercised: the handler, the service call, and the mapping from a repository refusal to a 404 rather than a 403. The half that is not is the resolution of a session to an owner, which is a single function returning a constant until authentication exists. The rejected alternative — a header naming the owner — would have tested it end to end at the cost of shipping an authentication bypass with a comment promising its removal. | Delete the override when authentication lands and issue the requests as two registered users. The table is already written; only how the owner is established changes, which is the point of keeping that in one function. |
