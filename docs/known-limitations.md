...
| M13 | **The row said one hardcoded `"Plantopia"`. A full accounting for this change found seven in Python and four more in a parallel TypeScript table.** The rejection copy this row named (`agent/nodes/intake.py`) is fixed, folded into `U8` since both needed the same function touched. `api/errors.py`'s two — the busy-queue and daily-cap error details — are fixed by Task 7's `register(app, settings)`. `services/chat_events.py`'s four (`TOOL_NAMES`) are deliberately left. | Closing those four means the server side reading `app_title`, and it cannot without breaking something else: `web/src/screens/chat/sources.ts:10-16` holds the identical four strings as a static TypeScript literal, and `tests/api/test_source_names_agree.py` regex-parses that file to assert the two tables say the same thing — deliberately two tables, in two languages, neither able to import the other. A dynamic server side would agree with a client that has no settings concept at all only by coincidence. So this is a *product-rename* task wearing a naming-tidy-up's name, not the few lines its own row promised. | Serve the title to the client — an endpoint, or bake it in at build time — so both tables can agree on one variable instead of one literal. Only then does making `chat_events.py` dynamic mean anything. |
