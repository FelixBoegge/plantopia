...
- [x] 6.1 Remove the location field from the upload step; verify a component test asserts a run starts without anybody being asked where the plant is.
...
- [x] 7.4 Update the README: what is read, what is discarded, the new settings, and the attribution obligation; verify every command runs as written on a clean clone.
- [x] 7.5 Record what this change leaves undone — anything detected that is still unused, and the privacy surface it opens; verify identifiers and dates against the file's conventions.
- [x] 7.6 Run `openspec validate add-image-metadata-capture --strict`, the Python suite, the frontend suite, Playwright and ruff; verify all five are clean.
