# chat-context Specification

## Purpose
How much of a conversation reaches the model, what may be dropped from it, and what must
survive.

Every turn resends what came before, so an unbounded history is a cost that compounds rather
than accumulates: turn *N* pays for turns 1..*N*. What makes this tractable is that the weight
is not evenly spread — tool output is roughly six times what a person and the agent actually
said, and it is also the most disposable, because a search made twenty turns ago is not what
the current question is about.

So the capability is ordered by cost: drop what is bulky and cheap to lose, and only then
condense what somebody said, keeping recent exchanges whole. Underneath both sits the
constraint that gives the rest its meaning — the stored transcript is never altered. What a
person reads is a record of what was said, and an owner has been promised a copy of it.
