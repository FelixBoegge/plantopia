...
## Purpose
What a plant history shows, in what order, and dated by what. A diagnosis is a verdict on one
moment; the question an owner has after the second or third is whether the plant is getting
better, and that is a question about sequence — what was seen, what it was judged to be, what
was done, and what the weather was doing.

Two constraints shape the whole capability. Events are dated by when they *happened* rather
than when they were recorded, because a photograph uploaded three days late would otherwise
sit in an order the plant never experienced. And an event renders from whatever it holds:
most observations predate capture dates and weather entirely, so a timeline that required
them would be a timeline that worked only for plants photographed after a particular week.
